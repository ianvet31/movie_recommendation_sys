...
from myfuns import (genres, get_displayed_movies, get_popular_movies,
                    get_recommended_movies, genreRecommendation, myIBCF)
...
sidebar = html.Div(
    [
        html.H3("Movie Recommendation Systems", className="display-8"),
        html.Hr(),
        dbc.Nav(
            [
            ...
            dbc.Row(
                [
                    html.Div(
                        [
                            *[
                                get_movie_cardRating(movie)
                                for idx, movie in genreRecommendation(genre).iterrows()
                            ],
                        ],
                        className="row row-cols-1 row-cols-5",
                    ),
...
def get_movie_cardRating(movie, with_rating=False):
    return html.Div(
        dbc.Card(
            [
                dbc.CardImg(
                ...
                dbc.CardBody(
                    [
                        html.H6("Adjusted rating: "+str(round(movie.AvgRating, 2)), className="card-title text-center"),
                        html.H6("# of reviews: "+str(movie.NumRatings), className="card-title text-center"),
                    ]
                ),
            ]
            + (
...
if __name__ == "__main__":    app.run_server(port=8080, debug=True)